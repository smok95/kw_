...
static void OnEventConnect(long errCode);

static void OnReceiveTrData(PCSTR sScrNo, PCSTR sRQName,
	PCSTR sTrCode, PCSTR sRecordName, PCSTR sPrevNext, long nDataLength,
	PCSTR sErrorCode, PCSTR sMessage, PCSTR sSplmMsg);

static void OnReceiveRealData(PCSTR sRealKey, PCSTR sRealType, PCSTR sRealData);

int example_trRealRecv()
{
	kw_Initialize(0);
